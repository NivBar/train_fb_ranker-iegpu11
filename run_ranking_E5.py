...
if __name__ == "__main__":
    ...
    # Greg Data Edition
    # orig = pd.read_csv('greg_data.csv').drop('position', axis=1)
    #
    # query_dict = orig[['query_id', 'query']].drop_duplicates().set_index('query_id').to_dict('index')
    #
    # dfs = []
    # for key, df in tqdm(orig.groupby(['round_no', 'query_id'])):
    #     query_str = query_dict[key[1]]['query']
    #     new_df = rank_documents(query_str, df, text_col = 'current_document')
    #     dfs.append(new_df)
    # final_df = pd.concat(dfs).reset_index(drop=True)
    # final_df = final_df.sort_values(by=['round_no', 'query_id','rank']).rename({'rank':'position'}, axis=1).drop('score', axis=1)
    # final_df.to_csv(f'./greg_data_e5.csv', index=False)